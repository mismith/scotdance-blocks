...
<template>
  <span
    ref="el"
    data-judge-chip
    class="group/chip flex min-w-0 items-center cursor-grab rounded bg-judge px-2 py-1 text-xs font-medium leading-tight text-judge-foreground select-none glass glass-judge has-[[data-grip]:focus-visible]:z-10 has-[[data-grip]:focus-visible]:ring-2 has-[[data-grip]:focus-visible]:ring-ring comfortable:px-3 comfortable:py-1.5 comfortable:text-sm"
    :class="{ 'opacity-40': isDragging }"
    :title="label"
  >
    <span data-grip tabindex="0" class="mr-1 -ml-1 opacity-50 outline-none select-none">⠿</span
    ><span class="max-w-64 flex-1 truncate"><slot>{{ label }}</slot></span>
    <button
